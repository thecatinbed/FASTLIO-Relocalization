...
bool   level_publish_en = false;
double lidar_install_roll_deg = 0.0;
double lidar_install_pitch_deg = 0.0;
double lidar_install_yaw_deg = 0.0;
M3D    R_level(Eye3d);
double init_offset_x = 0.0;
double init_offset_y = 0.0;
double init_offset_z = 0.0;
...
ros::Publisher pubLaserCloudDeskewed;  // 发布去畸变点云给 ICP mapper
...
void updatePublishRotation()
{
    const double roll_rad = lidar_install_roll_deg * M_PI / 180.0;
    const double pitch_rad = lidar_install_pitch_deg * M_PI / 180.0;
    const double yaw_rad = lidar_install_yaw_deg * M_PI / 180.0;

    R_level = (Eigen::AngleAxisd(yaw_rad, V3D::UnitZ()) *
               Eigen::AngleAxisd(pitch_rad, V3D::UnitY()) *
               Eigen::AngleAxisd(roll_rad, V3D::UnitX())).toRotationMatrix();
}
...
/*** [MODIFIED] 修改fastPredictIMU函数，添加拉平和偏置支持 ***/
...
void publish_frame_world(const ros::Publisher & pubLaserCloudFull)
{
    ...
    if (pcd_save_en)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            /*** [MODIFIED] PCD保存也使用修正后的转换（RGBpointBodyToWorld已包含拉平和偏置） ***/
...
void publish_frame_deskewed(const ros::Publisher & pub_deskewed)
{
    ...
    pub_deskewed.publish(laserCloudmsg);
}
...
int main(int argc, char** argv)
{
    ...
    nh.param<bool>("publish/level_publish_en", level_publish_en, false);
    nh.param<double>("publish/lidar_roll_deg", lidar_install_roll_deg, 0.0);
    nh.param<double>("publish/lidar_pitch_deg", lidar_install_pitch_deg, 0.0);
    nh.param<double>("publish/lidar_yaw_deg", lidar_install_yaw_deg, 0.0);
    nh.param<double>("publish/lidar_install_roll_deg", lidar_install_roll_deg, lidar_install_roll_deg);
    nh.param<double>("publish/lidar_install_pitch_deg", lidar_install_pitch_deg, lidar_install_pitch_deg);
    nh.param<double>("publish/lidar_install_yaw_deg", lidar_install_yaw_deg, lidar_install_yaw_deg);
    nh.param<double>("publish/init_offset_x", init_offset_x, 0.0);
    nh.param<double>("publish/init_offset_y", init_offset_y, 0.0);
    nh.param<double>("publish/init_offset_z", init_offset_z, 0.0);
    ...
    if(level_publish_en) {
        updatePublishRotation();
        ROS_INFO("======================================");
        ROS_INFO("Lidar install attitude publish correction ENABLED!");
        ROS_INFO("  roll/pitch/yaw: [%.2f, %.2f, %.2f] deg",
                 lidar_install_roll_deg, lidar_install_pitch_deg, lidar_install_yaw_deg);
        ROS_INFO("  Init offset: [%.3f, %.3f, %.3f]", init_offset_x, init_offset_y, init_offset_z);
        ROS_INFO("======================================");
    } else {
        R_level = Eye3d;
        if(init_offset_x != 0.0 || init_offset_y != 0.0 || init_offset_z != 0.0) {
            ROS_INFO("======================================");
            ROS_INFO("Init offset enabled: [%.3f, %.3f, %.3f]", init_offset_x, init_offset_y, init_offset_z);
            ROS_INFO("======================================");
        }
    }
    ...
    // 新增：发布去畸变点云
    pubLaserCloudDeskewed = nh.advertise<sensor_msgs::PointCloud2>
            ("/points_deskewed", 100000);
...
//------------------------------------------------------------------------------------------------------
    ...
    while (status)
    {
        ...
        if(sync_packages(Measures)) 
        {
            ...
            publish_frame_deskewed(pubLaserCloudDeskewed);
